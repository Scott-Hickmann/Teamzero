...
export default withAuth(async function handler(
  ...
  const viewproperties = allProperties.map((properties) => documentToObject(properties));
  ...
  const viewDonations = allDonations.map((donation) => documentToObject(donation));
  ...
  res.json({ success: true, data: { viewDonations, viewPeople, viewproperties } });
});
